...
class _ShardWriter:
    ...
    def write(self, sample: dict):
        key = sample["__key__"]
        if self._tar is None or self._count >= self.max_count or (
...
class WebDatasetOutput:
    ...
    def run(self, context):
        ...
        max_count = cfg.output.max_shard_count
        max_size = cfg.output.max_shard_size
        ...
        with _ShardWriter(output_dir, max_count=max_count, max_size=max_size) as sink:
            for _, row in df.iterrows():
                ...
                if processor == "video2pose":
                    ...
                    paths = {"mp4": raw_dir / f"{sample_id}.mp4"}
                else:
                ...
                if not all(path and path.exists() for path in paths.values()):
                    skipped += 1
                    continue

                sample = {
                    "__key__": str(sample_id),
                    **{ext: path.read_bytes() for ext, path in paths.items()},
                    "txt": row_value(row, "TEXT"),
                }
        ...
        if not written:
            self.logger.warning("No processed samples were available to package.")
        self.logger.info(
